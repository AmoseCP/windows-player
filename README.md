...
- **文件夹视图**：侧栏「音乐库」下按磁盘目录层级展示，只显示目录名（完整路径见悬停提示），
  点某个文件夹只看该目录及子目录的曲目；右键可播放整个文件夹、批量加入歌单、
  在文件管理器中打开、把该目录下的曲目移出音乐库
- **音乐文件夹**：右键「音乐库」可添加一个或多个根文件夹作为库来源，启动时自动扫描、
  也可手动重新扫描；新增文件自动入库，被移动或改名的文件按大小重新匹配（歌单引用不断），
  真正消失的标灰，手动删除过的不会再被扫回来
- **从链接导入**：顶栏「从链接导入」填写指向音频文件本身的直链（如教会网站上的 .mp3），
  下载到「音乐 / Bethel Church Audio Player」并加入音乐库；仅支持直接音频链接，
  不解析 YouTube 等流媒体页面
...
本播放器仅供个人学习与教会内部使用，不用于商业运作。应用本身不提供任何音乐内容：
本地曲目由使用者自行导入，在线播放由 YouTube 官方页面提供，播放器不下载、不转存
流媒体内容。请确保导入与播放的音频已获得合法授权；在聚会等公开场合使用录制音乐，
可能另需版权方或相应版权机构（如 CCLI）的许可。
